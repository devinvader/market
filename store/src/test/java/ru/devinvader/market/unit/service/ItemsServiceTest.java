...
public class ItemsServiceTest {
    ...
    void getItems_emptyResult_returnsEmptyRows() {
        ...
        // when
        Mono<PagedListItemDto> result = itemsService.getItems(SEARCH, SORT_TYPE, PAGE, SIZE, USER_ID);
    ...
    void getItems_oneItem_returnsOneGroupWithTwoEmpty() {
        ...
        when(cartService.getItemCounts(anyLong(), anyList())).thenReturn(Mono.just(Map.of(1L, 2)));
        ...
        verify(cartService).getItemCounts(anyLong(), itemIdsCaptor.capture());
    ...
    void getItems_threeItems_returnsOneGroupWithoutEmpty() {
        ...
        when(cartService.getItemCounts(anyLong(), anyList())).thenReturn(Mono.just(Map.of(1L, 1, 2L, 0, 3L, 5)));
    ...
    void getItems_fourItems_returnsTwoGroups_secondGroupHasOneItemAndTwoEmpty() {
        ...
        when(cartService.getItemCounts(anyLong(), anyList())).thenReturn(Mono.just(Map.of()));
    ...
    void getItem_existingItemWithCartCount_returnsItemDto() {
        ...
        when(cartService.getItemCounts(anyLong(), anyList())).thenReturn(Mono.just(Map.of(itemId, 3)));

        ItemDto expectedDto = new ItemDto(itemId, "Item 1", "Desc 1", 1000L, 3);

        // when
        Mono<ItemDto> result = itemsService.getItem(itemId, USER_ID);
        ...
        verify(cartService).getItemCounts(anyLong(), eq(List.of(itemId)));
    }
    ...
    void getItems_pagination_hasPreviousAndNext() {
        ...
        // when
        Mono<PagedListItemDto> result = itemsService.getItems(SEARCH, SORT_TYPE, 2, 5, USER_ID);
    ...
    void getItems_pagination_noNext() {
        ...
        // when
        Mono<PagedListItemDto> result = itemsService.getItems(SEARCH, SORT_TYPE, 2, 10, USER_ID);
    ...
    void getItem_cachedItem_returnsFromCacheWithoutDbCall() {
        ...
        when(cartService.getItemCounts(anyLong(), anyList())).thenReturn(Mono.just(Map.of(itemId, 1)));
    ...
    void getItems_cachedList_returnsFromCacheWithoutDbCall() {
        ...
        when(cartService.getItemCounts(anyLong(), anyList())).thenReturn(Mono.just(Map.of(1L, 0)));
    ...
    void getItems_withSearch_usesShorterTtl() {
        ...
        // when
        itemsService.getItems(search, SORT_TYPE, PAGE, SIZE, USER_ID).block();
